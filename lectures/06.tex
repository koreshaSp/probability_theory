...
\begin{proof}[\normalfont\textsc{Доказательство (Туран)}]
 ...
 \end{align*}
 \begin{align*}
	 \D\xi_p = \underbrace{\E\xi_p^{2}}_{=\E \xi_p} - (\E\xi_p)^{2} = \frac{1}{p} + O \left( \frac{1}{n} \right) - \left(\frac{1}{p} + O(\frac{1}{n})\right)^{2} = \frac{1}{p} - \frac{1}{p^{2}} + O \left( \frac{1}{n} \right).
 \end{align*} Тогда
 \begin{align*}
